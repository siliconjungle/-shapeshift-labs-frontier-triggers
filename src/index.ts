...
export type TriggerCondition =
  | TriggerPathCondition
  | TriggerCapabilityCondition
  | TriggerScopeCondition
  | TriggerSubjectCondition
...
export type TriggerCapabilitySource =
  | readonly string[]
  | Set<string>
  | {
...
export type TriggerActionInputFactory = (
  event: TriggerEvent,
  context: TriggerEvaluationContext
) => JsonValue | undefined;

export type TriggerActionFactory = (
  event: TriggerEvent,
  context: TriggerEvaluationContext
) => TriggerActionBinding | readonly (TriggerActionBinding | string)[] | string | false | null | undefined;
...
export type TriggerEventEmissionFactory = (
  event: TriggerEvent,
  context: TriggerEvaluationContext,
  outcome: TriggerOutcome
) => TriggerEventEmission | readonly TriggerEventEmission[] | false | null | undefined;

export type TriggerEventEmissionSource =
  | TriggerEventEmission
  | readonly TriggerEventEmission[]
  | TriggerEventEmissionFactory;
...
export interface TriggerRuntime {
  ...
  replay(events: readonly TriggerEventInput[], options?: TriggerEmitOptions & { reset?: boolean }): TriggerReplayResult;
  history(): TriggerRecord[];
  clearHistory(): void;
  snapshot(options?: { includeHistory?: boolean }): TriggerSnapshot;
  restore(snapshot: TriggerSnapshot, options?: { definitions?: boolean; gates?: boolean; history?: boolean }): void;
...
interface NormalizedTriggerDefinition extends TriggerDefinition {
  event: TriggerEventPattern;
  requires: readonly string[];
  when: readonly TriggerCondition[];
  subjects: readonly TriggerSubjectMatcher[];
...
interface TriggerMatch {
  trigger: NormalizedTriggerDefinition;
  context: TriggerEvaluationContext;
}

interface TriggerActionRunResult {
  outcomes: TriggerOutcome[];
  emissions: PendingTriggerEmission[];
}

interface PendingTriggerEmission {
  trigger: NormalizedTriggerDefinition;
  event: TriggerEvent;
  context: TriggerEvaluationContext;
  outcome: TriggerOutcome;
  source: TriggerEventEmissionSource;
}

interface TriggerEmitInternalOptions extends TriggerEmitOptions {
  cascadeDepth?: number;
}
...
export function defineTrigger(definition: TriggerDefinition): TriggerDefinition {
  return cloneDefinition(definition);
}

export function createTriggerRegistryGraph(runtime: TriggerRuntime): FrontierRegistryGraph {
  return runtime.registryGraph();
}

export function traceTriggerImpact(runtime: TriggerRuntime, input: FrontierRegistryImpactInput): FrontierRegistryImpact {
  return runtime.impact(input);
}

export function normalizeTriggerEvent(input: TriggerEventInput, options: { now?: number; source?: string; sequence?: number } = {}): TriggerEvent {
  ...
  return {
    ...
    metadata: input.metadata === undefined ? undefined : cloneJson(input.metadata) as JsonObject,
    cancelable: input.cancelable ?? true
  };
}

class FrontierTriggerRuntime implements TriggerRuntime {
  private readonly definitions = new Map<string, NormalizedTriggerDefinition>();
  private readonly records: TriggerRecord[] = [];
  private readonly onceKeys = new Set<string>();
  private readonly cooldowns = new Map<string, { triggerId: string; key: string; lastAt: number; retryAt: number }>();
  private readonly maxHistory: number;
  private nextEventSequence = 1;
  private nextOutcomeSequence = 1;
  private nextOrder = 1;

  constructor(private readonly options: TriggerRuntimeOptions = {}) {
  ...
  get(id: string): TriggerInfo {
    ...
    if (!trigger) throw new TypeError('unknown trigger: ' + id);
  ...
  emit(input: TriggerEventInput, options: TriggerEmitOptions = {}): TriggerEmitResult {
    return this.emitInternal(input, { ...options, require: options.require ?? false });
  }

  require(input: TriggerEventInput, options: TriggerEmitOptions = {}): TriggerEmitResult {
  ...
  replay(events: readonly TriggerEventInput[], options: TriggerEmitOptions & { reset?: boolean } = {}): TriggerReplayResult {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      for (const record of result.records) records[records.length] = cloneRecord(record);
  ...
  snapshot(options: { includeHistory?: boolean } = {}): TriggerSnapshot {
    const cooldowns: TriggerCooldownSnapshot[] = [];
    for (const entry of this.cooldowns.values()) cooldowns[cooldowns.length] = { ...entry };
    return {
      kind: 'frontier.triggers.snapshot',
      version: 1,
  ...
  restore(snapshot: TriggerSnapshot, options: { definitions?: boolean; gates?: boolean; history?: boolean } = {}): void {
    ...
    if (options.definitions ?? true) {
      ...
      for (const info of snapshot.definitions) {
        this.definitions.set(info.id, this.normalizeDefinition(info as TriggerDefinition));
      }
    }
    if (options.gates ?? true) {
      this.onceKeys.clear();
      for (const key of snapshot.onceKeys) this.onceKeys.add(key);
    ...
    if (options.history ?? snapshot.records !== undefined) {
      this.records.length = 0;
      for (const record of snapshot.records ?? []) this.pushRecord(record);
    }
  }

  inspect(): TriggerInspectGraph {
    const nodes: TriggerInspectGraphNode[] = [];
    const edges: TriggerInspectGraphEdge[] = [];
    for (const trigger of this.definitions.values()) {
      ...
      nodes[nodes.length] = {
        ...
        kind: 'trigger',
        label: trigger.id,
        metadata: trigger.metadata === undefined ? undefined : cloneJson(trigger.metadata) as JsonObject
      };
  ...
  private emitInternal(input: TriggerEventInput, options: TriggerEmitInternalOptions): TriggerEmitResult {
    ...
    const emissions: PendingTriggerEmission[] = [];
    ...
    for (const trigger of ordered) {
      ...
      if (exclusiveKey && exclusiveGroups.has(exclusiveKey)) {
        outcomes[outcomes.length] = this.outcome(
          trigger,
          event,
          'rejected',
          makeRejection('exclusive-conflict', 'trigger exclusive group already handled this event', trigger.id, event.id, event.type),
          now
        );
        continue;
      }
      ...
      if (exclusiveKey) exclusiveGroups.add(exclusiveKey);
      ...
      if (actionResult.outcomes.length === 0) {
        ...
        for (const outcome of actionResult.outcomes) outcomes[outcomes.length] = outcome;
        for (const emission of actionResult.emissions) emissions[emissions.length] = emission;
      }
      if (trigger.consume) {
        consumed = true;
        if (event.cancelable) break;
      }
    }
  ...
  private finalizeEmit(
    event: TriggerEvent,
    outcomes: TriggerOutcome[],
    accepted: boolean,
    consumed: boolean,
    rejection: TriggerRejection | undefined,
    options: TriggerEmitOptions
  ): TriggerEmitResult {
    ...
    for (const action of actions) {
      ...
      try {
        ...
        if (mode === 'schedule' && scheduler) {
          ...
          if (actionOptions.autoRun) requestSchedulerRun(scheduler, actionOptions.runOptions);
        ...
        if (backendHasAction === false) {
          outcomes[outcomes.length] = this.outcome(
            ...
            event,
            'rejected',
            makeRejection('action-unavailable', 'trigger action is not registered: ' + actionId, trigger.id, event.id, event.type),
            now,
            actionId
          );
          continue;
        ...
        outcomes[outcomes.length] = this.outcome(
          ...
          event,
          'rejected',
          makeRejection('action-unavailable', 'trigger has no action backend for: ' + actionId, trigger.id, event.id, event.type),
          now,
          actionId
        );
      } catch (error) {
        outcomes[outcomes.length] = this.outcome(
          ...
          event,
          'failed',
          makeRejection('action-failed', errorMessage(error), trigger.id, event.id, event.type),
          now,
          actionId
        );
      }
  ...
  private collectEmissions(
    ...
    event: TriggerEvent,
    context: TriggerEvaluationContext,
    action: TriggerActionBinding,
    outcome: TriggerOutcome
  ): void {
    if (action.emit === undefined) return;
    emissions[emissions.length] = { trigger, event, context, outcome, source: action.emit };
  }

  private emitCascades(emissions: PendingTriggerEmission[], options: TriggerEmitInternalOptions): TriggerRecord[] {
    ...
    for (const emission of emissions) {
      ...
      for (const eventInput of events) {
        ...
        for (const record of result.records) records[records.length] = record;
      }
    }
    return records;
  }

  private rejectCascade(
    input: TriggerEventInput,
    emission: PendingTriggerEmission,
    options: TriggerEmitInternalOptions
  ): TriggerEmitResult {
    const now = normalizeTimestamp(options.now ?? this.now());
    let event: TriggerEvent;
    try {
    ...
    const rejection = makeRejection(
      'cascade-depth',
      'trigger cascade depth limit reached',
      emission.trigger.id,
      event.id,
      event.type,
      {
  ...
  private scheduledOutcome(
    ...
    event: TriggerEvent,
    actionId: string,
    options: TriggerActionScheduleOptions,
    handle: unknown,
    now: number
  ): TriggerOutcome {
  ...
  private createEvaluationContext(
    trigger: NormalizedTriggerDefinition,
    event: TriggerEvent,
    capabilities: TriggerCapabilitySource | undefined,
    state: unknown,
    now: number
  ...
  private evaluateOnce(trigger: NormalizedTriggerDefinition, event: TriggerEvent, context: TriggerEvaluationContext): TriggerRejection | undefined {
    const onceKey = trigger.once ? event.type + ':' + (event.subject ?? event.id) : resolveKey(trigger.oncePer, event, context);
    if (!onceKey) return undefined;
  ...
  private markOnce(trigger: NormalizedTriggerDefinition, event: TriggerEvent, context: TriggerEvaluationContext): void {
    const onceKey = trigger.once ? event.type + ':' + (event.subject ?? event.id) : resolveKey(trigger.oncePer, event, context);
    if (onceKey) this.onceKeys.add(trigger.id + '\0' + onceKey);
  }
  ...
  private evaluateCooldown(
    ...
    if (!trigger.cooldownMs || trigger.cooldownMs <= 0) return undefined;
  ...
  private markCooldown(trigger: NormalizedTriggerDefinition, event: TriggerEvent, context: TriggerEvaluationContext, now: number): void {
    if (!trigger.cooldownMs || trigger.cooldownMs <= 0) return;
    const cooldownKey = resolveKey(trigger.cooldownKey, event, context) ?? event.type + ':' + (event.subject ?? event.id);
  ...
  private emitRecord(record: TriggerRecord, options: TriggerEmitOptions): void {
    ...
    try {
      if (record.accepted) logger?.info?.('frontier.triggers.emit', cloned as unknown as Record<string, unknown>);
      else logger?.error?.('frontier.triggers.emit', cloned as unknown as Record<string, unknown>);
    } catch {}
    const eventLog = options.eventLog ?? this.options.eventLog;
    if (eventLog) {
      appendEventLog(
        eventLog,
        { type: 'frontier.triggers.emit', record: recordToJson(cloned) },
        record.event.id,
        { eventType: record.event.type, accepted: record.accepted },
        options.eventLogMode ?? this.options.eventLogMode ?? 'frontier-event-log'
      );
    }
  }

  private emitOutcome(outcome: TriggerOutcome): void {
    try {
      this.options.onOutcome?.(cloneOutcome(outcome));
    } catch {}
  }

  private pushRecord(record: TriggerRecord): void {
    this.records[this.records.length] = cloneRecord(record);
    while (this.records.length > this.maxHistory) this.records.shift();
  }

  private normalizeDefinition(definition: TriggerDefinition): NormalizedTriggerDefinition {
...
function evaluateStaticMatch(trigger: NormalizedTriggerDefinition, event: TriggerEvent): TriggerRejection | undefined {
...
function evaluateCondition(condition: TriggerCondition, event: TriggerEvent, context: TriggerEvaluationContext): TriggerConditionResult {
  if ('test' in condition && typeof condition.test === 'function') {
    ...
    if (isRejection(result)) return { accepted: false, rejection: result };
    return result;
  }
  ...
  if ('scope' in condition) return { accepted: matchesScope(event.scope, condition.scope) };
  if ('subject' in condition) return { accepted: matchesSubject(event, condition.subject) };
  if ('all' in condition || 'any' in condition || 'not' in condition) {
    ...
    if (condition.any) {
      let any = false;
      let firstRejection: TriggerRejection | undefined;
      for (const item of condition.any) {
        const result = evaluateCondition(item, event, context);
        any = any || result.accepted;
        firstRejection ??= result.rejection;
      }
      if (!any) return { accepted: false, rejection: firstRejection };
    }
    if (condition.not) {
  ...
  const pathCondition = condition as TriggerPathCondition;
  const value = context.read(pathCondition.path);
  if (pathCondition.exists !== undefined && (value !== undefined) !== pathCondition.exists) return { accepted: false };
  if (pathCondition.truthy !== undefined && Boolean(value) !== pathCondition.truthy) return { accepted: false };
  if (pathCondition.equals !== undefined && !jsonEqual(value, pathCondition.equals)) return { accepted: false };
  if (pathCondition.notEquals !== undefined && jsonEqual(value, pathCondition.notEquals)) return { accepted: false };
  if (pathCondition.oneOf !== undefined && !pathCondition.oneOf.some((item: JsonValue) => jsonEqual(value, item))) return { accepted: false };
  if (pathCondition.gt !== undefined && !(typeof value === 'number' && value > pathCondition.gt)) return { accepted: false };
  if (pathCondition.gte !== undefined && !(typeof value === 'number' && value >= pathCondition.gte)) return { accepted: false };
  if (pathCondition.lt !== undefined && !(typeof value === 'number' && value < pathCondition.lt)) return { accepted: false };
  if (pathCondition.lte !== undefined && !(typeof value === 'number' && value <= pathCondition.lte)) return { accepted: false };
  return { accepted: true };
}

function resolveEmittedEvents(emission: PendingTriggerEmission): TriggerEventInput[] {
  const value = typeof emission.source === 'function'
    ? emission.source(emission.event, emission.context, emission.outcome)
    : emission.source;
  if (!value) return [];
  const items = Array.isArray(value) ? value : [value];
  const out: TriggerEventInput[] = [];
...
function normalizeEmittedEvent(input: TriggerEventEmission, emission: PendingTriggerEmission): TriggerEventInput {
  const metadata = mergeRecordMetadata(
    {
      parentEventId: emission.event.id,
...
function resolveActions(
  ...
  const actions: TriggerActionBinding[] = [];
  for (const action of trigger.actions ?? []) actions[actions.length] = normalizeActionBinding(action);
  const action = trigger.action;
  if (typeof action === 'string') {
    actions[actions.length] = { id: action };
  } else if (typeof action === 'function') {
    const result = action(event, context);
    if (typeof result === 'string') actions[actions.length] = { id: result };
    else if (isActionBindingArray(result)) {
      for (const item of result) actions[actions.length] = typeof item === 'string' ? { id: item } : cloneActionBinding(item);
...
function resolveActionInput(
  ...
  if (typeof value === 'function') return cloneOptionalJson(value(event, context));
  if (value !== undefined) return cloneOptionalJson(value);
  return cloneOptionalJson(event.payload);
}

function normalizeActionBinding(input: string | TriggerActionBinding): TriggerActionBinding {
  return typeof input === 'string' ? { id: input } : cloneActionBinding(input);
}

function isActionBindingArray(value: unknown): value is readonly (TriggerActionBinding | string)[] {
  return Array.isArray(value);
}

function cloneActionBinding(input: TriggerActionBinding): TriggerActionBinding {
  return {
    ...input,
...
function cloneEmissionSource(source: TriggerEventEmissionSource | undefined): TriggerEventEmissionSource | undefined {
  if (source === undefined || typeof source === 'function') return source;
  if (Array.isArray(source)) return source.map((item) => cloneEmission(item));
  return cloneEmission(source as TriggerEventEmission);
}

function cloneEmission(input: TriggerEventEmission): TriggerEventEmission {
  return {
    ...input,
    scope: input.scope === undefined ? undefined : cloneScope(input.scope),
...
function hasCapability(
  source: TriggerCapabilitySource | undefined,
  capability: string,
  event: TriggerEvent,
  context: TriggerCapabilityContext
): boolean {
  if (!capability) return true;
  if (!source) return false;
  if (Array.isArray(source)) return source.includes(capability);
  if (source instanceof Set) return source.has(capability);
  return (source as { has(capability: string, event: TriggerEvent, context: TriggerCapabilityContext): boolean }).has(capability, event, context);
}

function mergeCapabilities(
  left: TriggerCapabilitySource | undefined,
  right: TriggerCapabilitySource | undefined
): TriggerCapabilitySource | undefined {
...
function matchesEventPattern(pattern: TriggerEventPattern, type: string): boolean {
  const patterns = patternList(pattern);
  for (const item of patterns) {
    if (item === '*' || item === type) return true;
    if (item.endsWith('*') && type.startsWith(item.slice(0, -1))) return true;
  }
  return false;
}

function matchesScope(scope: TriggerScope | undefined, matcher: TriggerScopeMatcher): boolean {
  if (!scope) return false;
  if (matcher.kind !== undefined && scope.kind !== matcher.kind) return false;
  if (matcher.id !== undefined && scope.id !== matcher.id) return false;
  if (matcher.tag !== undefined && !(scope.tags ?? []).includes(matcher.tag)) return false;
  for (const tag of matcher.tags ?? []) {
    if (!(scope.tags ?? []).includes(tag)) return false;
  }
  return true;
}

function matchesSubject(event: TriggerEvent, matcher: TriggerSubjectMatcher): boolean {
  if (matcher.subject !== undefined && event.subject !== matcher.subject) return false;
  if (matcher.subject !== undefined && Object.keys(matcher).length === 1) return true;
  for (const subject of event.subjects) {
    if (matcher.kind !== undefined && subject.kind !== matcher.kind) continue;
    if (matcher.id !== undefined && subject.id !== matcher.id) continue;
    if (matcher.role !== undefined && subject.role !== matcher.role) continue;
    if (matcher.tag !== undefined && !(subject.tags ?? []).includes(matcher.tag)) continue;
    let tagsMatch = true;
    for (const tag of matcher.tags ?? []) {
...
function readContextPath(root: unknown, path: TriggerPath): unknown {
...
function normalizePath(path: TriggerPath): (string | number)[] {
...
function triggerInfo(trigger: NormalizedTriggerDefinition): TriggerInfo {
  return {
    id: trigger.id,
...
function triggerToRegistryEntry(trigger: NormalizedTriggerDefinition) {
  ...
  return {
    ...
    version: trigger.version,
    contentHash: trigger.contentHash,
    source: cloneSourceLocation(trigger.sourceLocation) as FrontierRegistrySource | undefined,
...
function outcomeToRegistryRecord(record: TriggerRecord, outcome: TriggerOutcome): FrontierRegistryRecord {
  return {
...
function recordToJson(record: TriggerRecord): JsonObject {
  ...
  if (record.rejection !== undefined) out.rejection = rejectionToJson(record.rejection);
  return out;
}

function eventToJson(event: TriggerEvent): JsonObject {
  const out: JsonObject = {
    id: event.id,
    type: event.type,
    source: event.source,
  ...
  if (event.scope !== undefined) out.scope = scopeToJson(event.scope);
  if (event.actor !== undefined) out.actor = event.actor;
  if (event.causeId !== undefined) out.causeId = event.causeId;
  if (event.tick !== undefined) out.tick = event.tick;
  if (event.payload !== undefined) out.payload = cloneJson(event.payload);
  if (event.metadata !== undefined) out.metadata = cloneJson(event.metadata);
  return out;
}

function outcomeToJson(outcome: TriggerOutcome): JsonObject {
  const out: JsonObject = {
    id: outcome.id,
  ...
  if (outcome.actionId !== undefined) out.actionId = outcome.actionId;
  if (outcome.rejection !== undefined) out.rejection = rejectionToJson(outcome.rejection);
  ...
  if (outcome.metadata !== undefined) out.metadata = cloneJson(outcome.metadata);
  return out;
}

function rejectionToJson(rejection: TriggerRejection): JsonObject {
  const out: JsonObject = {
    code: rejection.code,
    message: rejection.message
  };
  if (rejection.triggerId !== undefined) out.triggerId = rejection.triggerId;
  if (rejection.eventId !== undefined) out.eventId = rejection.eventId;
  if (rejection.eventType !== undefined) out.eventType = rejection.eventType;
  if (rejection.capability !== undefined) out.capability = rejection.capability;
  if (rejection.condition !== undefined) out.condition = cloneJson(rejection.condition);
  if (rejection.retryAt !== undefined) out.retryAt = rejection.retryAt;
  if (rejection.metadata !== undefined) out.metadata = cloneJson(rejection.metadata);
  return out;
}
...
function appendEventLog(
  ...
  key: string,
  headers: JsonObject,
  mode: TriggerEventLogMode
): void {
  try {
    if (mode === 'raw') eventLog.append(value);
    else eventLog.append({ value, key, headers });
  } catch {}
}

function requestSchedulerRun(scheduler: TriggerSchedulerLike, options: unknown): void {
  if (typeof scheduler.requestRun === 'function') scheduler.requestRun(options);
  else if (typeof scheduler.run === 'function') scheduler.run(options);
}

function resolveKey(
  key: string | TriggerKeyFactory | undefined,
  event: TriggerEvent,
  context: TriggerEvaluationContext
): string | undefined {
...
function normalizeSubjects(
  subject: string | TriggerSubjectMatcher | undefined,
  subjects: readonly TriggerSubjectMatcher[] | undefined
): TriggerSubjectMatcher[] {
  const out: TriggerSubjectMatcher[] = [];
  if (typeof subject === 'string') out[out.length] = { subject };
  else if (subject) out[out.length] = cloneSubjectMatcher(subject);
  for (const item of subjects ?? []) out[out.length] = cloneSubjectMatcher(item);
  return out;
}

function normalizeEventPattern(pattern: TriggerEventPattern): TriggerEventPattern {
  if (typeof pattern !== 'string') return normalizeStringList(pattern);
  return normalizePattern(pattern);
}

function patternList(pattern: TriggerEventPattern): string[] {
  return typeof pattern !== 'string' ? pattern.map(normalizePattern) : [normalizePattern(pattern)];
}

function normalizePattern(pattern: string): string {
  if (!isNonEmptyString(pattern)) throw new TypeError('trigger event pattern must be a non-empty string');
  return pattern;
}
...
function normalizeStringList(values: readonly string[] | undefined): string[] {
  ...
  for (const value of values ?? []) {
    if (isNonEmptyString(value) && !out.includes(value)) out[out.length] = value;
  }
  return out;
}

function normalizeId(id: string, label: string): string {
  if (!isNonEmptyString(id)) throw new TypeError(label + ' must be a non-empty string');
  return id;
}
...
function normalizeHistoryLimit(value: number | undefined): number {
  if (value === 0) return 0;
  if (!Number.isFinite(value ?? NaN)) return DEFAULT_MAX_HISTORY;
  return Math.max(0, Math.floor(value as number));
}

function normalizeCascadeDepthLimit(value: number | undefined): number {
  if (!Number.isFinite(value ?? NaN)) return DEFAULT_MAX_CASCADE_DEPTH;
  return Math.max(0, Math.floor(value as number));
}

function compareTriggers(left: NormalizedTriggerDefinition, right: NormalizedTriggerDefinition): number {
...
function makeRejection(
  ...
  return {
    ...
    capability,
    retryAt,
    metadata: metadata === undefined ? undefined : cloneJson(metadata) as JsonObject
  };
}

function isRejection(value: unknown): value is TriggerRejection {
  return value !== null && typeof value === 'object' && typeof (value as TriggerRejection).code === 'string' && typeof (value as TriggerRejection).message === 'string';
}
...
function cloneEvent(event: TriggerEvent): TriggerEvent {
  return {
    id: event.id,
    type: event.type,
    source: event.source,
    ...
    metadata: event.metadata === undefined ? undefined : cloneJson(event.metadata) as JsonObject,
    cancelable: event.cancelable
  };
}

function cloneScope(scope: TriggerScope): TriggerScope {
  return {
    kind: normalizeId(scope.kind, 'trigger scope kind'),
    id: normalizeOptionalString(scope.id),
    tags: scope.tags?.slice(),
    metadata: scope.metadata === undefined ? undefined : cloneJson(scope.metadata) as JsonObject
  };
}

function cloneSubject(subject: TriggerSubject): TriggerSubject {
  return {
    ...
    fields: subject.fields === undefined ? undefined : cloneJson(subject.fields) as JsonObject
  };
}

function subjectToJson(subject: TriggerSubject): JsonObject {
  const out: JsonObject = { kind: subject.kind, id: subject.id };
  if (subject.role !== undefined) out.role = subject.role;
  if (subject.tags !== undefined) out.tags = subject.tags.slice();
  if (subject.fields !== undefined) out.fields = cloneJson(subject.fields);
  return out;
}

function scopeToJson(scope: TriggerScope): JsonObject {
  const out: JsonObject = { kind: scope.kind };
  if (scope.id !== undefined) out.id = scope.id;
  if (scope.tags !== undefined) out.tags = scope.tags.slice();
  if (scope.metadata !== undefined) out.metadata = cloneJson(scope.metadata);
  return out;
}

function cloneSubjectMatcher(matcher: TriggerSubjectMatcher): TriggerSubjectMatcher {
  return {
    ...matcher,
...
function cloneSubjectMatcherInput(subject: string | TriggerSubjectMatcher | undefined): string | TriggerSubjectMatcher | undefined {
  if (typeof subject === 'string' || subject === undefined) return subject;
  return cloneSubjectMatcher(subject);
}

function cloneSourceLocation(
  source: TriggerSourceLocation | readonly TriggerSourceLocation[] | undefined
): TriggerSourceLocation | TriggerSourceLocation[] | undefined {
  if (source === undefined) return undefined;
  if (Array.isArray(source)) return source.map((item) => ({ ...item }));
  return { ...(source as TriggerSourceLocation) };
}
...
function cloneRecord(record: TriggerRecord): TriggerRecord {
  return {
    id: record.id,
...
function cloneOutcome(outcome: TriggerOutcome): TriggerOutcome {
  return {
    ...outcome,
    ...
    metadata: outcome.metadata === undefined ? undefined : cloneJson(outcome.metadata) as JsonObject
  };
}

function cloneScheduled(scheduled: TriggerScheduledAction): TriggerScheduledAction {
  return { ...scheduled };
}

function cloneRejection(rejection: TriggerRejection): TriggerRejection {
  return {
    ...rejection,
    condition: rejection.condition === undefined ? undefined : cloneJson(rejection.condition),
    metadata: rejection.metadata === undefined ? undefined : cloneJson(rejection.metadata) as JsonObject
  };
}

function mergeJson(left: JsonObject | undefined, right: JsonObject | undefined): JsonObject | undefined {
  if (left === undefined) return right === undefined ? undefined : cloneJson(right) as JsonObject;
  if (right === undefined) return cloneJson(left) as JsonObject;
  return { ...(cloneJson(left) as JsonObject), ...(cloneJson(right) as JsonObject) };
}

function mergeRecordMetadata(...items: (JsonObject | Record<string, unknown> | undefined)[]): JsonObject | undefined {
  const out: JsonObject = {};
  for (const item of items) {
    if (!item) continue;
    for (const [key, value] of Object.entries(item)) {
      if (value !== undefined && isJsonValue(value)) out[key] = cloneJson(value);
    }
  }
  return Object.keys(out).length === 0 ? undefined : out;
}

function conditionToJson(condition: TriggerCondition): JsonValue {
  if ('path' in condition) {
    const out: JsonObject = { path: typeof condition.path === 'string' ? condition.path : condition.path.map(String).join('.') };
    if (condition.exists !== undefined) out.exists = condition.exists;
    if (condition.equals !== undefined) out.equals = cloneJson(condition.equals);
    if (condition.notEquals !== undefined) out.notEquals = cloneJson(condition.notEquals);
    if (condition.oneOf !== undefined) out.oneOf = condition.oneOf.map((value) => cloneJson(value));
    if (condition.truthy !== undefined) out.truthy = condition.truthy;
    if (condition.gt !== undefined) out.gt = condition.gt;
    if (condition.gte !== undefined) out.gte = condition.gte;
    if (condition.lt !== undefined) out.lt = condition.lt;
    if (condition.lte !== undefined) out.lte = condition.lte;
    return out;
  }
  if ('capability' in condition) return { capability: condition.capability };
  if ('scope' in condition) return { scope: matcherToJson(condition.scope) };
  if ('subject' in condition) return { subject: matcherToJson(condition.subject) };
  if ('test' in condition) return { predicate: true };
  const out: JsonObject = {};
  if (condition.all) out.all = condition.all.map(conditionToJson);
  if (condition.any) out.any = condition.any.map(conditionToJson);
  if (condition.not) out.not = conditionToJson(condition.not);
  return out;
}

function matcherToJson(matcher: TriggerScopeMatcher | TriggerSubjectMatcher): JsonObject {
  const out: JsonObject = {};
...
function staticActionIds(trigger: TriggerDefinition): string[] {
  const ids: string[] = [];
  for (const action of trigger.actions ?? []) {
    const id = typeof action === 'string' ? action : action.id;
    if (isNonEmptyString(id) && !ids.includes(id)) ids[ids.length] = id;
  }
  if (typeof trigger.action === 'string' && !ids.includes(trigger.action)) ids[ids.length] = trigger.action;
...
function staticEmissionTypes(trigger: TriggerDefinition): string[] {
  const types: string[] = [];
  for (const action of trigger.actions ?? []) collectStaticEmissionTypes(typeof action === 'string' ? undefined : action.emit, types);
  if (trigger.action && typeof trigger.action === 'object') collectStaticEmissionTypes(trigger.action.emit, types);
  return types;
}

function collectStaticEmissionTypes(source: TriggerEventEmissionSource | undefined, types: string[]): void {
  if (!source || typeof source === 'function') return;
  const items = Array.isArray(source) ? source : [source];
  for (const item of items) {
    if (isNonEmptyString(item.type) && !types.includes(item.type)) types[types.length] = item.type;
  }
}

function hasRuntimeCallbacks(trigger: TriggerDefinition): boolean {
...
function actionHasCallback(action: TriggerActionBinding): boolean {
...
function conditionHasCallback(condition: TriggerCondition): boolean {
  if ('test' in condition) return true;
  if ('all' in condition && condition.all?.some(conditionHasCallback)) return true;
  if ('any' in condition && condition.any?.some(conditionHasCallback)) return true;
  if ('not' in condition && condition.not && conditionHasCallback(condition.not)) return true;
  return false;
}

function pathToRegistryPath(path: TriggerPath): string | readonly (string | number)[] {
  return Array.isArray(path) ? path.slice() : path;
}

function scopeMatcherKey(scope: TriggerScopeMatcher): string {
  return [scope.kind ?? '*', scope.id ?? '*', ...(scope.tags ?? []), scope.tag ?? ''].filter(Boolean).join(':');
}

function subjectMatcherKey(subject: TriggerSubjectMatcher): string {
  return [subject.subject ?? '', subject.kind ?? '*', subject.role ?? '*', subject.id ?? '*', ...(subject.tags ?? []), subject.tag ?? ''].filter(Boolean).join(':');
}

function dedupeNodes(nodes: TriggerInspectGraphNode[]): TriggerInspectGraphNode[] {
  const seen = new Set<string>();
  const out: TriggerInspectGraphNode[] = [];
...
function dedupeInspectEdges(edges: TriggerInspectGraphEdge[]): TriggerInspectGraphEdge[] {
  const seen = new Set<string>();
  const out: TriggerInspectGraphEdge[] = [];
...
function jsonEqual(left: unknown, right: unknown): boolean {
  ...
  if (isPlainObject(left) || isPlainObject(right)) {
    ...
    for (let i = 0; i < leftKeys.length; i++) {
      ...
      if (key !== rightKeys[i] || !jsonEqual(left[key], right[key])) return false;
    }
    return true;
  }
  return false;
...
function stableEventSuffix(type: string, subject: string | undefined): string {
  let hash = 2166136261;
